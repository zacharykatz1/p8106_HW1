...
We choose the model with the minimum training RMSE to apply to our predictions. This model's training RMSE is `r train_mse_lm`.
...
We choose the model with the minimum training RMSE to apply to our predictions. This model's training RMSE is `r train_mse_lm_caret`. Then, we'd measure the performance of our best model on the test data, finding the same test RMSE as we found using the first method.
...
For each of other computational approaches (`glmnet` or `caret`), we may decide to use the lambda that minimizes RMSE or the lambda 1SE. We'll try both.
...
# Use 1se rule
cv_lasso_glmnet_1se$lambda.1se
```
...
# Use 1se rule
cv_lasso_glmnet_min$lambda.min
```
...
# Make predictions using glmnet object
lasso_predict_min = predict(cv_lasso_glmnet_min, newx = testing_predictors_matrix, s = "lambda.min", type = "response")

rmse_lasso_glmnet_min = RMSE(lasso_predict_min, test_data$sale_price)

rmse_lasso_glmnet_min
```
...
# Optimal lambda 1SE
lasso_caret_fit_1se$bestTune$lambda
```
...
rmse_lasso_caret_1se
```

#### Lambda min rule

In `caret`, we may alternatively decide to use the more traditional lambda tuning that minimizes RMSE. We implement this similarly to the above, as follows:
...
# Set cross-validation parameters for 1SE
control_cv_min = trainControl(method = "repeatedcv", number = 20, repeats = 5)
...
# Optimal lambda 1SE
lasso_caret_fit_min$bestTune$lambda
```
...
# Resampling testing
resamp = resamples(list(lm = fit_lm_caret, lasso_1se = lasso_caret_fit_1se, lasso_min = lasso_caret_fit_min, enet = enet_fit))
...
Based on this, we would likely use the elastic net model to predict our response because its mean RMSE over cross-validation trials is lower than the mean RMSE found for the other methods.